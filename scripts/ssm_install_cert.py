...
def send_ssm_command(instance_id: str, script_content: str, domain: str, email: str, region: str = "us-east-1"):
    ...
    result = subprocess.run(cmd, capture_output=True, text=True, check=True)  # noqa: S603 - static aws-cli argv, no shell
...
def wait_for_command(instance_id: str, command_id: str, region: str = "us-east-1", max_wait: int = 300):
    ...
    while time.time() - start_time < max_wait:
        ...
        result = subprocess.run(cmd, capture_output=True, text=True)  # noqa: S603 - static aws-cli argv, no shell
